Feature: Deactivate medical program
  ...
  Scenario: Successful deactivation of a medical program
  ...
  Scenario: Deactivate medical program with active program
    Given the following medical programs exist:
      | databaseId                             |
  ...
  Scenario: Deactivate medical program with read scope
  ...
  Scenario: Deactivate medical program with MSP client
  ...
  Scenario: Deactivate medical program with invalid id
    Given my scope is "medical_program:write"
    And my client type is "NHS"
    And my consumer ID is "a538013e-70e6-4554-83ef-8d2beedbe7b0"
