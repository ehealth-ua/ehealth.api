defmodule Core.ContractRequests do
  ...
  def create_from_draft(headers, params) do
    ...
    with {:ok, contractor_legal_entity} <- LegalEntities.fetch_by_id(client_id),
         ...
         content <- Map.put(content, "contractor_legal_entity_id", contractor_legal_entity.id),
         {:ok, content, contract} <- validate_contract_number(pack.type, content, headers),
         :ok <- validate_contractor_legal_entity_id(contractor_legal_entity.id, contract),
         pack <- RequestPack.put_decoded_content(pack, content),
         :ok <- validate_previous_request(pack, contractor_legal_entity.id),
  ...
  def update(headers, %{"id" => _} = params) do
    ...
    with :ok <- validate_update_params(pack),
         ...
         {:ok, contractor_legal_entity} <- LegalEntities.fetch_by_id(contract_request.contractor_legal_entity_id),
         :ok <- validate_contractor_legal_entity_status(contractor_legal_entity),
         :ok <- validate_contractor_legal_entity_nhs_verification(contractor_legal_entity),
         update_params <-
  ...
  def sign_nhs(headers, %{"id" => id} = params) do
    ...
    with {:ok, nhs_legal_entity} <- LegalEntities.fetch_by_id(client_id),
         ...
         :ok <- validate_legal_entity_edrpou(nhs_legal_entity, signer),
         :ok <- validate_legal_entity_edrpou(nhs_legal_entity, stamp),
  ...
  def sign_msp(headers, client_type, %{"id" => _, "type" => _} = params) do
    ...
    with %LegalEntity{} = contractor_legal_entity <- LegalEntities.get_by_id(client_id),
         ...
         :ok <- validate_legal_entity_edrpou(contractor_legal_entity, signer_msp),
