defmodule Core.Persons do
  ...
  def search(params) do
    with %Ecto.Changeset{valid?: true, changes: changes} <- Search.changeset(params),
      ...
      {:ok, %{paging: %{total_pages: total_pages}}} -> {:error, %{"paging" => %{"total_pages" => total_pages}}}
      error -> error
    end
  end
  ...
  def update(id, params, headers) do
    ...
    with {:ok, %{"data" => user}} <- @mithril_api.get_user_by_id(user_id, headers),
         ...
         {:ok, person} <- get_by_id(user["person_id"]),
  ...
  def search_persons(params, fields \\ nil, ops \\ []) do
    with {:ok, persons} <- @rpc_worker.run("mpi", MPI.Rpc, :search_persons, [params, fields, ops]) do
      {:ok, persons}
    else
  ...
  def list(filter, order_by \\ [], cursor \\ nil) do
    with {:ok, persons} <- @rpc_worker.run("mpi", MPI.Rpc, :ql_search, [filter, order_by, cursor]) do
      {:ok, persons}
    end
  end
