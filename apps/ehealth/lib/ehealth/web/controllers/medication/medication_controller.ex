defmodule EHealth.Web.MedicationController do
  ...
  def create(conn, medication_params) do
    ...
    with {:ok, %Medication{} = medication} <- API.create_medication(medication_params, consumer_id) do
      conn
      |> put_status(:created)
  ...
  def deactivate(conn, %{"id" => id}) do
    ...
    with {:ok, %Medication{} = medication} <- API.deactivate_medication(medication, actor_id) do
      render(conn, "show.json", medication: medication)
    end
  end
end
