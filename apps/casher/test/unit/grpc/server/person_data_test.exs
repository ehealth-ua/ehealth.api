    ...
    test "success by employee_id" do
      ...
      assert %PersonDataResponse{person_ids: [^person_id]} =
               PersonData.person_data(
                 PersonDataRequest.new(employee_id: employee.id),
                 %Stream{}
               )
    end

    test "success by user_id, client_id" do
      party1 = insert(:prm, :party, birth_date: ~D[2000-01-03])
      party2 = insert(:prm, :party, birth_date: ~D[2000-01-02])
      ...
      assert %PersonDataResponse{person_ids: [^person_id]} =
               PersonData.person_data(
                 PersonDataRequest.new(user_id: party_user.user_id, client_id: employee.legal_entity_id),
                 %Stream{}
               )
    end
  end
